 ...
 * TranslinkS8Animator
 *
 * Handles section-specific scroll animations for S8.
 * DRAMATIC parallax content reveal: adds 3D tilt and significant scale.
 */
export class TranslinkS8Animator {
    private tls: gsap.core.Timeline[] = [];

    setup(_parent: HTMLElement): void {
        const section = document.getElementById('s8');
        if (!section) return;

        this.destroy();

        const headline = section.querySelector('h2');
        const stats = section.querySelector('.grid, #s8-stats');
