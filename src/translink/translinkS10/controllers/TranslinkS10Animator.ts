...
/**
 * TranslinkS10Animator
 *
 * Handles section-specific scroll animations for S10.
 * DRAMATIC parallax content reveal: adds 3D tilt and significant scale.
 */
export class TranslinkS10Animator {
    private tls: gsap.core.Timeline[] = [];

    setup(_parent: HTMLElement): void {
        const section = document.getElementById('s10');
        if (!section) return;

        this.destroy();

        const headline = section.querySelector('h2');
        const stats = section.querySelector('.grid, #s10-stats');
