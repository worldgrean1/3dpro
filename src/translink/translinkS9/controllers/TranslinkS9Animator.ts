 ...
 * TranslinkS9Animator
 *
 * Handles section-specific scroll animations for S9.
 * DRAMATIC parallax content reveal: adds 3D tilt and significant scale.
 */
export class TranslinkS9Animator {
    private tls: gsap.core.Timeline[] = [];

    setup(_parent: HTMLElement): void {
        const section = document.getElementById('s9');
        if (!section) return;

        this.destroy();

        const headline = section.querySelector('h2');
        const stats = section.querySelector('.grid, #s9-stats');
