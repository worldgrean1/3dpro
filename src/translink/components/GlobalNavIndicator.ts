...
export class GlobalNavIndicator {
    mount(parent: HTMLElement): void {
        const nav = document.createElement('nav');
        nav.id = 'global-nav-indicator';
        nav.className =
            'fixed right-4 md:right-10 top-1/2 -translate-y-1/2 w-[18vh] h-[65vh] flex flex-col items-center justify-center z-[var(--z-ui-global)] pointer-events-none';
        ...
        nav.innerHTML = `
            <div class="nav-indicator-container h-full w-full relative flex items-center justify-center pointer-events-auto cursor-pointer">
                <!-- Sized using viewport height units to span beautifully and rotate without bounding cuts -->
                <div class="absolute w-[55vh] h-[16vh] rotate-90 scale-x-[-1] flex items-center justify-center">
                    <svg class="w-full h-full overflow-visible" viewBox="0 0 304 112" preserveAspectRatio="xMidYMid meet">
                        <!-- Faint background track guide -->
                        <path id="nav-track-path" d="${suzukaPath}" stroke="rgba(255, 255, 255, 0.15)" fill="none" stroke-width="2.5"></path>
                        <!-- Dynamic active scroll progress drawing overlay in brand crimson -->
                        <path id="nav-track-progress" d="${suzukaPath}" stroke="#c0202f" fill="none" stroke-width="3" opacity="0.9"></path>
                        <!-- Dynamic directional vehicle chevron pointer driven by scroll percentage -->
                        <g id="nav-track-car" class="telemetry-track-dot-group">
                ...
                <div class="scroll-percentage absolute bottom-0 left-1/2 -translate-x-1/2 font-['Inter',sans-serif] text-[12px] md:text-[14px] font-black tracking-tight text-white/70 flex items-baseline gap-[1px]">
                    <span class="percentage-value font-mono min-w-[20px] text-right">0</span>
                    <span class="percentage-symbol text-[8px] opacity-40">%</span>
                </div>
            </div>
        `;
        ...
        requestAnimationFrame(() => {
            const percentIndicator = nav.querySelector('.percentage-value');
            const pathEl = nav.querySelector('#nav-track-path') as SVGPathElement;
            const progressPathEl = nav.querySelector('#nav-track-progress') as SVGPathElement;
            const carEl = nav.querySelector('#nav-track-car') as SVGGraphicsElement;

            if (!percentIndicator || !pathEl || !progressPathEl || !carEl) return;

            // Initialize progress stroke path parameters
            const pathLength = pathEl.getTotalLength();
            progressPathEl.style.strokeDasharray = `${pathLength}`;
            progressPathEl.style.strokeDashoffset = `${pathLength}`;

            // Create normalized static Anime.js motion-path animation instance
            ...
            // Register with the global scroll observer to link scroll progress to navigation track states
            scrollSystem.registerScrollNode(
                document.body,
                {
                    onProgress: (progress: number) => {
                        const percentage = Math.round(progress * 100);
                        percentIndicator.textContent = percentage.toString();

                        // 1. Seek the vehicle chevron along the track
                        carAnimation.progress = progress;

                        // 2. Draw the active progress line overlay in crimson
                        const offset = pathLength - progress * pathLength;
                        progressPathEl.style.strokeDashoffset = offset.toString();
                    },
                },
                {
                    endTrigger: '#s10',
