class VoiceTelemetryService {
    sessions = new Map();
    recentEvents = [];
    maxSessions = Number(process.env.VOICE_TELEMETRY_MAX_SESSIONS || 200);
    maxEvents = Number(process.env.VOICE_TELEMETRY_MAX_EVENTS || 1000);
    maxClientMetricsPerSession = Number(process.env.VOICE_TELEMETRY_MAX_CLIENT_METRICS || 80);
    ...
    increment(sessionId, counter, by = 1) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
    ...
    recordClientMetric(sessionId, metric) {
        const session = this.sessions.get(sessionId);
        if (!session || typeof metric.name !== 'string')
            return;
    ...
    closeSession(sessionId, closeCode, closeReason) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
    ...
    getSummary() {
        ...
        const avgDurationMs = completed.length === 0
            ? 0
            : Math.round(completed.reduce((sum, session) => sum + ((session.closedAt || session.startedAt) - session.startedAt), 0) / completed.length);
        return {
            ...
            totalClientInterruptions: sessions.reduce((sum, session) => sum + session.counters.clientInterruptions, 0),
            totalClientAudioStreamEnds: sessions.reduce((sum, session) => sum + session.counters.clientAudioStreamEnds, 0),
            totalSocketErrors: sessions.reduce((sum, session) => sum + session.counters.socketErrors, 0),
            totalMicPermissionFailures: sessions.reduce((sum, session) => sum + session.counters.micPermissionFailures, 0),
            totalPlaybackInterruptions: sessions.reduce((sum, session) => sum + session.counters.playbackInterruptions, 0),
        };
    }
    applyClientMetric(session, name, value) {
    ...
    recordEvent(sessionId, name, data) {
        const event = {
